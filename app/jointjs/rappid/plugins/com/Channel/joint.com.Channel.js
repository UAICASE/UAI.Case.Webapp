...
joint.com.Channel.prototype.sendMessage = function(message) {

    if (this._paused || this._closed) return;

    this.trigger('message:sent', message);

    if (this.debugLevel > 1) this.log('sendMessage', message);

    var target;

    while (target = message.target.shift()) {

        var receivingSite = this.sites[target];
        ...
        if (!receivingSite) continue;
        if (!receivingSite.socket) continue;
        // readyState constants: See https://developer.mozilla.org/en-US/docs/Web/API/WebSocket#Ready_state_constants.
        // 1 === OPEN (The connection is open and ready to communicate.)
        if (receivingSite.socket.readyState !== 1) continue;

        if (this.debugLevel > 1) {
            this.log('sendMessage', target, message);
        }

        receivingSite.socket.send(JSON.stringify(message));
    }
...
joint.com.ChannelHub.prototype.onConnection = function(socket) {
    ...
    var channel = this.router(req, function(error, channel) {
